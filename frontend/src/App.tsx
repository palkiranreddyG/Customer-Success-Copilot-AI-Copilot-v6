...
function AppContent() {
  ...
  const loadAccounts = async () => {
    try {
      ...
      setAccounts(data);
      if (data.length > 0) {
        // If current selected account was deleted or updated, keep it, otherwise default to first
        const stillExists = selectedAccount ? data.find(a => a.id === selectedAccount.id) : null;
        if (stillExists) {
  ...
  const renderActivePage = () => {
    switch (activePage) {
      ...
      case 'accounts':
        return <AccountsPage onAccountsChange={loadAccounts} />;
      case 'analytics':
          ...
          <main className="p-6 lg:p-8 space-y-8">
            ...
            <section>
              <PlatformStatsPanel 
                ...
                activeRecommendations={statusData?.recommendations}
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-slate-550 flex items-center gap-2">
              ...
              {accountsError ? (
                <div className="p-4 rounded-2xl border border-rose-200 dark:border-rose-900/40 bg-rose-50 dark:bg-rose-950/20 flex items-start gap-3 text-sm text-rose-700 dark:text-rose-350">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8">
                  ...
                  <div className="space-y-3 max-h-[380px] overflow-y-auto pr-1">
                    {searchResults.length > 0 ? (
                      searchResults.map((result, idx) => {
                        const relevance = Math.max(0, Math.min(100, Math.round((1 - result.score / 2) * 100)));
                        return (
                          <div key={idx} className="card-lift p-4 rounded-xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/30 hover:border-slate-200 dark:hover:border-slate-700 transition-colors animate-step-appear" style={{ animationDelay: `${idx * 50}ms` }}>
                            ...
                            <p className="text-sm text-slate-650 dark:text-slate-350 mt-2 leading-relaxed italic">
                              "{highlightKeywords(result.text, searchQuery)}"
                            </p>
                          </div>
                        );
                      })
              ...
              <div className="lg:col-span-2 flex flex-col gap-5">
                <TranscriptUpload
                  accounts={accounts}
                  onUploadSuccess={(sessionId, accountId) => {
